import { FormattedServiciosTable } from '@/lib/definitios'
import Image from 'next/image'
import Link from 'next/link'

export default async function ServiciosTable({
  servicios,
}: {
  servicios: FormattedServiciosTable[]
}) {
  return (
    <div className="w-full">
      <h1 className={`mb-8 text-xl md:text-2xl`}>
        Servicios
      </h1>
      {/* <Search placeholder="Search servicios..." /> */}
      <div className="mt-6 flow-root">
        <div className="overflow-x-auto">
          <div className="inline-block min-w-full align-middle">
            <div className="overflow-hidden rounded-md bg-gray-50 p-2 md:pt-0">
              <div className="md:hidden">
                {servicios?.map((servicio) => (
                  <div
                    key={servicio.id}
                    className="mb-2 w-full rounded-md bg-white p-4"
                  >
                    <div className="flex items-center justify-between border-b pb-4">
                      <div>
                        <div className="mb-2 flex items-center">
                          <div className="flex items-center gap-3">
                            <Image
                              src={servicio.image_url}
                              className="rounded-full"
                              alt={`${servicio.name}'s profile picture`}
                              width={28}
                              height={28}
                            />
                            <p>{servicio.name}</p>
                          </div>
                        </div>
                        <p className="text-sm text-gray-500">
                          {servicio.email}
                        </p>
                      </div>
                    </div>
                    <div className="flex w-full items-center justify-between border-b py-5">
                      <div className="flex w-1/2 flex-col">
                        <p className="text-xs">Pending</p>
                        <p className="font-medium">{servicio.total_pending}</p>
                      </div>
                      <div className="flex w-1/2 flex-col">
                        <p className="text-xs">Paid</p>
                        <p className="font-medium">{servicio.total_paid}</p>
                      </div>
                    </div>
                    <div className="pt-4 text-sm">
                      <p>{servicio.total_invoices} invoices</p>
                    </div>
                  </div>
                ))}
              </div>
              <table className="hidden min-w-full rounded-md text-gray-900 md:table">
                <thead className="rounded-md bg-gray-50 text-left text-sm font-normal">
                  <tr>
                    <th scope="col" className="px-4 py-5 font-medium sm:pl-6">
                      Order
                    </th>
                    <th scope="col" className="px-3 py-5 font-medium">
                      Ruta
                    </th>
                    <th scope="col" className="px-3 py-5 font-medium">
                      Precio de Servicio
                    </th>
                    <th scope="col" className="px-3 py-5 font-medium">
                      Chofer
                    </th>
                    <th scope="col" className="px-4 py-5 font-medium">
                      Total Paid
                    </th>
                  </tr>
                </thead>

                <tbody className="divide-y divide-gray-200 text-gray-900">
                  {servicios.map((servicio) => (
                    <tr key={servicio.id} className="group">
                      <td className="whitespace-nowrap bg-white py-5 pl-4 pr-3 text-sm text-black group-first-of-type:rounded-md group-last-of-type:rounded-md sm:pl-6">
                        <div className="flex items-center gap-3">

                          <Image
                            src="/icono.png"
                            className="rounded-full"
                            alt={`${servicio.ruta}'s ruta picture`}
                            width={36}
                            height={36}
                          />
                          <div className="flex-col">
                            <p>{servicio.orden}</p>
                            <p>{servicio.fecha_carga}</p>
                            <Link href="/" className='text-blue-500'> ver detalles</Link>
                          </div>
                        </div>
                      </td>
                      <td className="whitespace-nowrap bg-white px-4 py-5 text-sm">
                        {servicio.ruta}
                      </td>
                      <td className="whitespace-nowrap bg-white px-4 py-5 text-sm">
                        {servicio.precio_de_servicio}
                      </td>
                      <td className="whitespace-nowrap bg-white px-4 py-5 text-sm">
                        {servicio.chofer}
                      </td>
                      <td className="whitespace-nowrap bg-white px-4 py-5 text-sm group-first-of-type:rounded-md group-last-of-type:rounded-md">
                        {servicio.estatus_pago}
                      </td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}
